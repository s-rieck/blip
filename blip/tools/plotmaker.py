...
def plotmaker(post, params,parameters, inj, Model, Injection=None,saveto=None,config_kwargs={},plot_kwargs={}):
    ...
    cc.configure(smooth=False, kde=False, max_ticks=2, sigmas=np.array([1, 2]), label_font_size=18, tick_font_size=18, \
            summary=False, statistics="max_central", spacing=2, summary_area=0.95, cloud=False, bins=1.2,**config_kwargs)
    cc.configure_truth(color='g', ls='--', alpha=0.7)

    if knowTrue:
        fig = cc.plotter.plot(figsize=(19, 19), truth=truevals, **plot_kwargs) #-smr change these to make corner plot include what it should
    else:
        fig = cc.plotter.plot(figsize=(19, 19), **plot_kwargs)
